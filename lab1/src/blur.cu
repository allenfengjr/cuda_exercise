...
void cuda_blur_kernel_convolution(uint thread_index, const float* gpu_raw_data,
    ...
    // https://stackoverflow.com/questions/7104384/why-is-a-b010-better-than-if-else-version-in-cuda
    // I think nvcc may achieve a better performance using min than if-statement
    uint convSize = min(blur_v_size,thread_index+1);
...
float cuda_call_blur_kernel(const unsigned int blocks,
    ...
    float* gpu_raw_data;
    // Cpp will not change the value of a pointer paramenter, only the value that the pointer
    // point to will be changed. In that case, if I want to change a pointer's value
    // I must set the parameter as a dual-pointer. 
    // If I give a float* f, the function will only change
    // (*f) from 1.5 to 2.5, f will still be something like x001E.
    ...
    float* gpu_out_data;
    cudaMalloc(&gpu_out_data,sizeof(float)*n_frames);// no copy there
    printf("allocate all memory\n");
